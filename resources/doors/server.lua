...
AddEventHandler('vrpdoorsystem:open', function(id)
  ...
  if vRP.hasPermission({user_id, "#"..cfg.list[id].key..".>0"}) or vRP.hasPermission({user_id,cfg.list[id].permission}) then
    ...
    if cfg.list[id].locked then
      vRPclient.notify(player, {"Cerraste la puerta "..cfg.list[id].name})
    else
      vRPclient.notify(player, {"Abriste la puerta "..cfg.list[id].name})
    end
  else
    vRPclient.notify(player, {"No tenes permitido el ingreso "..cfg.list[id].name})
  end
end)
