...
AddEventHandler('chatMessage', function(source, name, msg)
    ...
    elseif msg:sub(1,3) == "/do" then
        ...
        if user_id ~= nil then
            vRP.getUserIdentity({user_id, function(identity)
                if identity ~= nil then
                    TriggerClientEvent('chatMessage', -1, "^6DO | "..identity.firstname.. " "..identity.name, { 10, 205, 245 }, msg:sub(4))
                end
            end})
        end
    ...
    elseif msg:sub(1,5) == "/anon" then
        CancelEvent()
        local user_id = vRP.getUserId({source})
        if user_id ~= nil then
            vRP.getUserIdentity({user_id, function(identity)
                if identity ~= nil then
                    TriggerClientEvent('chatMessage', -1, "", { 220, 209, 50 }, "^3Anónimo | ^0" .. msg:sub(6))
                end
            end})
        end
    end
