...
settings.welcomeMessage         =       GetConvar("vBasic.welcomeMessage", "Bienvenido ^1{player}^0 a ^2FALOPEROS ^3RP ^0Leete las reglas en discord: ^2discord.gg/xgZh7xb.")
...
settings.trafficDensity         =       tonumber(GetConvar("vBasic.trafficDensity", "0.1"))
settings.pedDensity             =       tonumber(GetConvar("vBasic.pedDensity", "0.1"))
...
PerformHttpRequest("https://vespura.com/vbasic-version.txt", function(errorCode, result, headers)
    local version = "v4.0"
    if (string.find(tostring(result), version) == nil) then
        print("\n\r[vBasic] WARNING: Your version of vBasic is not up to date. Please make sure to update whenever possible.\n\r")
    else
        print("\n\r[vBasic] You are running the latest version of vBasic. Good job!\n\r")
    end
end, "GET", "", "")
